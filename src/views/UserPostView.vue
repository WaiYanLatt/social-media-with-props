  ...
  >
    ...
    >
      <div class="relative">
        ...
        <div class="absolute top-[75%] z-10 ml-2 flex items-center">
          <img
            ...
            alt=""
            class="w-32 h-32 rounded-full object-cover border-2 border-sky-500"
          />
      ...
      <router-link :to="`/users/${currentUser.id}`">
        <button
          class="bg-slate-700 p-3 mt-10 text-white font-semibold active:bg-sky-500 w-[90%] ml-5 rounded-lg"
        >
          More Info
        </button>
      </router-link>
    ...
    <div class="lg:ml-96 z-20">
      <div
        class="w-full py-4 lg:py-3 fixed top-0 px-3 lg:px-0 lg:pl-44 bg rounded-none"
      >
        <button
          @click="showCard"
          class="lg:w-[50%] w-full md:w-[95%] p-3 lg:ml-5 md:ml-5 text-white font-semibold bg-sky-500 active:bg-slate-700 rounded-lg"
        >
          CREATE POST
        </button>
      </div>
      <div class="grid grid-cols-1 mb-10 lg:mt-20">
        <PostCardView
          ...
          v-for="post in currentUser.posts.slice().reverse()"
          :key="post.id"
          :post="post"
        ></PostCardView>
      </div>
    ...
    <div
      class="lg:w-[550px] lg:h-[370px] w-[350px] h-[350px] z-20 rounded-xl lg:p-10 p-5 bg-white shadow-2xl fixed top-32 lg:right-[30%]"
      v-show="show === true"
    >
      <Form
        @create="create"
        @unShow="unShow"
        :errorandsuccess="errorandsuccess"
      ></Form>
    </div>
  </div>
</template>
